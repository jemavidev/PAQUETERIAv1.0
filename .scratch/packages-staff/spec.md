...
- **Recibir**: campo de Guía **opcional** (entrada manual). **Entregar**: muestra el **destinatario snapshot** (solo lectura) para confirmar; menos campos, botones grandes. **Cancelar**: **dropdown de motivo obligatorio** + aviso de irreversibilidad.
...
- **Escaneo de código de barras con cámara (ZXing)** — la captura de Guía aquí es **manual**; el scan por cámara es la **siguiente mejora** sobre el modal Recibir (asset JS vendorizado, prueba manual/e2e). Documentado, no en esta rebanada.

...
- **Corrección post-deploy (2026-08-05):** la verificación "0 filas" de arriba chequeó una base RDS distinta y desactualizada (`paqueteria_staging`/`paqueteria_v4`, el sistema viejo de `customers`), no el Postgres real de `test.papyrus.com.co` (contenedor `paquetex-db-1`) donde vive este rebuild. Tras desplegar los 8 tickets se confirmó que ese Postgres SÍ tenía datos reales de pruebas previas: 19 `personas`, 12 `ocupantes`, 5 `apartamentos` (conjunto placeholder "CONJUNTO RESIDENCIAL LOS ROBLES", sin relación con "El Club") y 4 `usuarios` de staff. No hubo impacto — la migración de seed solo AGREGA las 804 filas nuevas, nunca toca ni borra las 5 existentes — pero la premisa "sin datos reales que reconciliar" de este documento era incorrecta para el ambiente que de verdad importa.

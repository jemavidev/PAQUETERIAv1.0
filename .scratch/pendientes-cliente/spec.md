...
- `316` — Bandera "primera entrega" (issue 314) faltaba en el modal Entregar duplicado de `/consultar` -- agregada + extraída a macro compartido para que los 2 templates no vuelvan a divergir; de paso se corrigió un bug de lógica invertida detectado en vivo — **implementado, pendiente verificar visualmente en vivo**
- `317` — "Agrupar por apartamento" con número exacto: mostrar SIEMPRE las 10 Torres fijas (layout 3+3+3+1, T10 centrada) -- cliente eligió Variante A (grid fijo) tras ver las 3 en prototipo, ya plegada al código real y prototipo borrado — **implementado, pendiente verificar visualmente en vivo**
- `318` — "Agrupar por apartamento" se activa solo (sin click aparte) al escribir "apt<número>" en la barra de búsqueda de `/residentes` -- JS puro, se revierte solo si el texto deja de matchear y nunca pisa un click manual — **implementado, sin poder verificar JS en vivo (sin navegador conectado)**
- `319` — Error 422 al presionar Enter en `/paquetes` -- submit nativo del form manda `conectados=` vacío, Pydantic rechazaba `bool` con cadena vacía -- corregido a `str` + comparación manual contra `"true"` — **implementado, pendiente verificar visualmente en vivo**
- `320` — Filtro "Sin apartamento asignado" en `/residentes` + botón por fila para asignar unidad directo desde la lista -- reusa el picker y la ruta ya existentes de la pestaña "Dirección", sin lógica de backend nueva — **implementado, pendiente verificar visualmente en vivo**
- `321` — Píldora cyan con el TOTAL de paquetes (todos los estados) de cada residente en `/residentes`, cliqueable, enlaza a `/paquetes?q=<teléfono/whatsapp/email>` con la misma búsqueda "exacta" de issue 308 -- lógica de conteo compartida con `packages.py` vía `condiciones_busqueda_paquetes` (relocada a `paquete_service.py`) para que conteo y clic-through nunca diverjan — **implementado, verificado contra datos reales, pendiente verificar visualmente en vivo**

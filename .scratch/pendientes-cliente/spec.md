...
- `263` — `/residentes` tab Residentes: bug real (duplicados por nombre sin validación) + hueco de UX (Ocupante sin contacto no tenía Confirmar/Rechazar) -- ahora sin contacto solo Confirmar/Rechazar mientras pending, Editar se habilita al confirmar (integra agregar el primer contacto, retira el form suelto), Notificaciones/Promover siguen exclusivos de quien tiene contacto propio, y `agregar_ocupante` rechaza nombres duplicados de Ocupantes activos sin contacto — **implementado** (issue 263 introdujo una regresión de [[259]], corregida y con test de regresión agregado el mismo día)
- `264` — `/residentes` tab Residentes, mobile: la fila nombre+iconos pasa a `flex-col` bajo `sm:` (los iconos bajan a su propia línea) y vuelve a una sola fila desde `sm:` (640px) — **implementado**
- `265` — Seguimiento a [[264]]: en mobile, la píldora Confirmado/Pendiente/Principal se separa al borde derecho de la tarjeta (`justify-between`, vuelve a `justify-start` desde `sm:`) — **implementado**
- `266` — `components/_modales.html::modal()`: ícono "✕" de cerrar `h-5 w-5` → `h-6 w-6` + `p-1` en el botón (área de click más grande) -- componente compartido, afecta todos los modales de la app — **implementado**
- `267` — `/residentes` y `/mis-datos`: la tab activa sobrevive un refresh/F5 -- el JS sincroniza `?tab=` en la URL en cada click (`history.replaceState`), el server ya lo leía en `/residentes` y ahora también en `/mis-datos` (nuevo, prioridad más baja que error/`ocupante_guardado`) — **implementado**
- `268` — `base.html` (toda la app): pulso visual breve (barra de 3px, `--site-brand`) SOLO en un refresh real (F5/botón recargar, detectado vía Navigation Timing), nunca en una navegación normal — **implementado**
- `269` — `/residentes` tab Dirección: 5 ajustes de texto -- se retira el aviso preventivo de issue 69 (`_aviso_reasignacion_bloqueada` eliminada entera, sus 2 mensajes eran las únicas ramas), el checkbox pasa a decir "Mudar residente de apartamento", y "Libre -- sin residentes registrados" → "Apartamento vacío." en el componente compartido (`_recibir_paquete.html`, afecta también /announce y /paquetes) — **implementado**
- `270` — Seguimiento a [[269]]: el checkbox "Mudar residente de apartamento" solo se muestra si `mi_ocupante` existe (mismo dato que la ruta ya usa para decidir si hay algo que mover) — **implementado**
- `271` — `/residentes` y `/mis-datos`: "Mudar residente de apartamento" y "Recibir paquetes sin autorización" pasan de checkbox nativo a toggle switch -- nuevo macro `components/_inputs.html::toggle()` (extendido con soporte `{% call %}` para el label rico de /mis-datos), Tailwind recompilado (`?v=66`) — **implementado**
- `272` — `mensaje_ya_ocupante_activo`: ya no cita literal "Mover acá" (desactualizado, y la función es compartida por 4 flujos con textos de checkbox distintos) -- describe la acción en vez de citar un botón puntual, fix de raíz — **implementado**
- `273` — Seguimiento a [[270]]/[[271]]: `mt-3` en la fila Guardar/Quitar de la tab Dirección para que el espaciado no dependa de si el toggle "Mudar residente" está visible — **implementado**
- `274` — Overflow en mobile (modales + tarjetas): `/mis-datos` nunca recibió la tanda de arreglos que sí tuvo `/residentes` (min-w-0/truncate/flex-col en el nombre y chips), portado ahora -- y en `_modales.html`, `overflow-x-hidden` agregado a ambos macros (causa real, `overflow-y-auto` solo dejaba `overflow-x` en `visible`, contenido podía salirse por los bordes redondeados) — **implementado**
- `275` — La columna "Acciones" de la lista de `/residentes` pasa al mismo estilo `chip_icono` (círculo con fondo de color) que ya usa la tab Residentes de la ficha -- y el ícono de basurero (ya usado ahí y en /paquetes) reemplaza el emoji "❌" en toda la app (ficha de staff icon-only, `/mis-datos` ícono+texto) — **implementado**
- `276` — Seguimiento a [[275]]: el ícono "Ver ficha" se retira de la lista de `/residentes` (3 ocurrencias en el archivo) -- el nombre del residente ya enlaza a la misma ficha en la misma fila — **implementado**
- `277` — `/residentes` tabla (mobile): columna Teléfono oculta, Torre y Apartamento en formato compacto ("T05-102"), nombre truncado, íconos de Acciones más chicos -- 0px de scroll lateral en un viewport de 359px (era 101px con solo los 2 cambios pedidos; el resto se agregó al verificar en vivo que no alcanzaba), desktop sin cambios — **verificado**
- `278` — Seguimiento a [[277]]: badges Auto/Principal bajan a su propia fila en mobile (`sm:contents`), nombre con ancho `clamp()` fluido (no un solo breakpoint, pedido explícito "lo mas ajustable posible"), íconos de Acciones con `clamp()` 26px→32px -- 0px de scroll desde 360px en adelante, desktop sin cambios — **verificado**
- `279` — Seguimiento a [[278]]: se retira el truncado "…" del nombre -- vuelve a mostrarse completo; en la práctica solo elimina el scroll lateral desde ~414px de ancho (a 360-390px reaparece, proporcional al largo del nombre), reportado al cliente con los números — **verificado**
- `280` — Seguimiento a [[279]]: nombre en mobile acotado a 2-3 palabras completas (nunca caracteres/"…") -- 3 palabras si esas 3 juntas no pasan de 20 caracteres, si no 2; nuevo helper `_nombre_mobile`, no toca el nombre real en base de datos, desktop sin cambios — **verificado**
- `281` — Seguimiento a [[280]]: columna Torre/Apto oculta en mobile -- su valor pasa a píldora gris junto a Auto/Principal (misma segunda fila de [[278]]); con la columna liberada, padding de Nombre/Acciones sube de `px-0.5` a `px-1` -- 0px de scroll, desktop sin cambios (píldora no se duplica, sigue viendo su columna dedicada) — **implementado**
- `282` — Seguimiento a [[281]]: píldora de Torre/Apto pasa de gris a `indigo`; sin apartamento asignado ya no se muestra la píldora (antes "No Asig."); gap/padding de las píldoras y entre filas sube (`gap-1`→`gap-1.5`, `px-1.5`→`px-2`) -- 0px de scroll, desktop sin cambios — **implementado**
- `283` — Seguimiento a [[282]]: Nombre/Acciones quedaban pegados al borde izquierdo en mobile -- padding `px-1`→`px-2` (4px→8px); 0px de scroll desde 375px, 12px remanente a 360px (mismo trade-off documentado en [[277]]), desktop sin cambios — **implementado**
- `284` — `autofocus` retirado de las 7 vistas exclusivas de staff que lo tenían (`/paquetes`, `/residentes`, ficha de residente, admin: usuarios/conjunto/notificaciones, `/announce` nuevo) -- robaba el foco en cada carga, molesto en un uso repetido a diario; NO tocado en vistas públicas/de cliente (`/anunciar` legado, `/buscar`, login/OTP de cliente) ni en login/reset de staff (se visitan una vez por sesión) — **implementado**
- `285` — Seguimiento a [[282]]: píldora de Torre/Apto pasa de `indigo` a `amber` -- se confundía con el azul de "Principal" en un badge tan chico; ahora Auto/Principal/Torre-Apto (verde/azul/ámbar) se distinguen a simple vista — **implementado**
- `286` — SMS de prueba no llegaba pese a que AWS confirmó salida del sandbox -- causa raíz encontrada (el sandbox es POR REGIÓN, solo `us-east-1` salió, las otras 9 probadas siguen en sandbox); agregado logging en `enviar_en_segundo_plano` (otp.py/notifications.py, antes tragaba la excepción sin rastro); la corrección de `AWS_REGION`/credenciales en el servidor queda pendiente del cliente (sin acceso SSH/Lightsail) — **diagnosticado, código corregido, infraestructura pendiente**
- `287` — Seguimiento a [[286]]: confirmado `AWS_REGION`/credenciales ya correctos en el servidor y AWS SNS funcional (envío directo por boto3 sí llegó); causa raíz real de por qué "Enviar prueba" (admin) no entregaba -- `admin_notificaciones_probar` nunca normalizaba el teléfono a E.164, AWS acepta un número sin `+57` y devuelve 200 igual pero el mensaje se pierde -- corregido con `normalizar_telefono()` (misma función que ya usa `/otp/solicitar`) — **verificado**
- `288` — Seguimiento a [[287]]: activado logging de entrega de AWS SNS (CloudWatch) para diagnosticar envíos "perdidos" -- resultaron ser SMS de 3 segmentos (14-20 min de demora) por tildes agudas (á/í/ó, ausentes del alfabeto GSM-7) forzando UCS-2; tildes retiradas de las 4 plantillas por defecto y de las 2 personalizadas en test -- RECIBIDO pasa de 3 segmentos UCS-2 a 1 solo GSM-7 — **implementado**
- `295` — Revierte parte de [[190]]: "Notificaciones"/"Proveedores" dejan de duplicarse como tab visible en `.site-nav` (staff admin) -- quedan únicamente en el menú de cuenta (`.site-actions`) — **verificado**
- `296` — Seguimiento a `.scratch/motivos-cancelacion-catalogo`: el CRUD de motivos de cancelación (renombrar/borrar) se unifica dentro del modal grande de cada fila CANCELADO -- antes vivía en una sección aparte, desconectada de las 3 plantillas de canal de ese mismo motivo — **implementado**
...
- `305` — Enlaces de WhatsApp: 2 links coexisten en el HTML (mobile `wa.me` / desktop `web.whatsapp.com` para PWA de Chrome), CSS decide cuál se ve según breakpoint, sin JS — **verificado**
- `306` — Botón flotante (`sticky bottom-0`, sin fondo, glow violeta) en los modales de confirmación de las vistas de staff (`/paquetes`, `/residentes`, `/administracion`) -- validado primero en "Recibir", replicado a los demás; pendiente decidir el caso especial de plantillas de notificación (2 botones por canal) — **implementado (salvo ese caso), verificación visual pendiente**
- `307` — Torre/Apto en rojo y sin link (modal "Ver" + columna "Dirección" de la tabla) cuando el destinatario de un paquete ya CERRADO se mudó de la unidad congelada en el snapshot -- reemplaza al ícono 🔄 de "cambio reciente" (limitado a 30 días), sin límite de tiempo — **implementado, verificación visual pendiente**
- `308` — Búsqueda "exacta" vs "conectada" (toggle) en Nombre/Teléfono/Email/WhatsApp -- "exacto" = campo propio del destinatario (o Anunciante == destinatario), "conectado" = solo coincide vía el Anunciante -- validado con datos reales (caso "JESUS VILLALOBOS": 6 exactos + 2 conectados por nombre) — **implementado, verificación visual pendiente**
- `309` — Buscar por Torre/Apto reusando el picker existente -- 3 rondas de UI (311, 312) no lograron convencer al cliente; pidió explícitamente remover todo — **removido**
- `310` — Botón "Mostrar conexiones" (issue 308) deshabilitado (real, no solo opacado) cuando el modo conectado no traería ningún resultado para la búsqueda actual — **implementado, pendiente verificar visualmente en vivo**
- `311` — Torre/Apto y "conectados" reubicados como íconos DENTRO del campo `q` -- la mitad Torre/Apto se removió con 309; la mitad "conectados" se mantiene — **conectados: implementado; Torre/Apto: removido**
- `312` — Torre/Apto reusa el ÚNICO campo `q` para el número de apartamento — **removido junto con 309**
- `313` — Ícono "Mostrar conexiones" muestra un badge ROJO con la cantidad de conexiones, visible ANTES de activar el toggle (adelanto, no confirmación) -- corregido en vivo desde una 1ra versión que solo lo mostraba ya activado y en índigo — **implementado, pendiente verificar visualmente en vivo**
- `314` — Bandera "primera entrega" en el modal Entregar (`/paquetes`) -- primera vez que se entrega un paquete a ese NÚMERO DE TELÉFONO específico, sin importar con quién viva — **implementado, pendiente verificar visualmente en vivo**
- `315` — Columna Acciones de `/residentes` siempre muestra los 4 íconos (WhatsApp/Llamar/Comparte apto/Eliminar) -- los que no aplican quedan apagados en vez de omitirse, para tamaño uniforme entre filas — **implementado, pendiente verificar visualmente en vivo**
- `316` — Bandera "primera entrega" (issue 314) faltaba en el modal Entregar duplicado de `/consultar` -- agregada + extraída a macro compartido para que los 2 templates no vuelvan a divergir; de paso se corrigió un bug de lógica invertida detectado en vivo — **implementado, desplegado a test.papyrus.com.co (2026-09-05), pendiente que el cliente lo confirme visualmente**
- `317` — "Agrupar por apartamento" con número exacto: mostrar SIEMPRE las 10 Torres fijas (layout 3+3+3+1, T10 centrada) -- cliente eligió Variante A (grid fijo) tras ver las 3 en prototipo, ya plegada al código real y prototipo borrado — **implementado, desplegado a test.papyrus.com.co (2026-09-05), pendiente que el cliente lo confirme visualmente**
- `318` — "Agrupar por apartamento" se activa solo (sin click aparte) al escribir "apt<número>" en la barra de búsqueda de `/residentes` -- JS puro, se revierte solo si el texto deja de matchear y nunca pisa un click manual — **implementado, desplegado a test.papyrus.com.co (2026-09-05), sin poder verificar JS en vivo (sin navegador conectado) -- pendiente que el cliente lo confirme**
- `319` — Error 422 al presionar Enter en `/paquetes` -- submit nativo del form manda `conectados=` vacío, Pydantic rechazaba `bool` con cadena vacía -- corregido a `str` + comparación manual contra `"true"` — **implementado, desplegado a test.papyrus.com.co (2026-09-05), pendiente que el cliente lo confirme visualmente**
- `320` — Filtro "Sin apartamento asignado" en `/residentes` + botón por fila para asignar unidad directo desde la lista -- reusa el picker y la ruta ya existentes de la pestaña "Dirección", sin lógica de backend nueva — **implementado, desplegado a test.papyrus.com.co (2026-09-05), pendiente que el cliente lo confirme visualmente**
- `321` — Píldora fucsia (cambiada desde cian tras pedido de más contraste) con el TOTAL de paquetes (todos los estados) de cada residente en `/residentes`, cliqueable, enlaza a `/paquetes?q=<teléfono/whatsapp/email>` con la misma búsqueda "exacta" de issue 308 -- lógica de conteo compartida con `packages.py` vía `condiciones_busqueda_paquetes` (relocada a `paquete_service.py`) para que conteo y clic-through nunca diverjan — **implementado, desplegado a test.papyrus.com.co (2026-09-05), verificado contra datos reales, pendiente que el cliente lo confirme visualmente**
- `322` — OTP (fuga de código en logs del monolito legado, no desplegado) y código de acceso (nunca visibles en vistas públicas no autenticadas -- `/anunciar/confirmacion` los exponía en pantalla y en la URL) — **implementado, desplegado a test.papyrus.com.co (2026-09-06), pendiente que el cliente lo confirme visualmente**
- `323` — `/paquetes`: buscar un código de acceso EXACTO trae también los paquetes `RECIBIDO` relacionados (mismo destinatario o misma unidad), para entregar todo de una sola consulta — **implementado, desplegado a test.papyrus.com.co (2026-09-06), pendiente que el cliente lo confirme visualmente**
- `324` — `/announce`: quitar el enlace "¿Solo registrar residentes?" del encabezado — **implementado, pendiente desplegar y confirmar**
- `325` — `/announce`: sección "Ya tiene paquetes en curso" solo lista `ANUNCIADO` (antes también `RECIBIDO`) -- esta vista es para anunciar/recibir, no para entregar/cancelar — **implementado, pendiente desplegar y confirmar**
- `326` — `/announce`: bandera `autoriza_recepcion_automatica` empieza a gatear el botón Recibir -- ON muestra píldora "Auto" + Recibir tal cual; OFF reemplaza Recibir por un link de WhatsApp pidiendo autorización (mensaje editable vía `PlantillaNotificacion`, nuevo pseudo-evento `PEDIR_AUTORIZACION`) — **implementado, pendiente desplegar y confirmar**
- `327` — `/announce`: 5 bugs/mejoras visuales -- texto del checkbox "mudar" con Torre/Apto concretos, botones Anunciar/Recibir duplicados al abrir "+ Nueva persona" (ahora se ocultan los de arriba vía JS), ícono en vez de "+ Nueva persona", lista de residentes en 2 columnas + nombre partido en 2 líneas, píldora "Auto" en rojo/blanco (antes verde sin contraste) — **implementado, pendiente desplegar y confirmar**
- `328` — `/announce`: "+ Nueva persona" reubicado ARRIBA de la tarjeta Anunciar/Recibir (antes iba debajo) — **implementado, pendiente desplegar y confirmar**
- `329` — `/announce`: badge fucsia con la cantidad de paquetes `ANUNCIADO` de cada residente en la lista de la unidad, mismo color que el pill de total de `/residentes` (issue 321) — **implementado, pendiente desplegar y confirmar**
- `330` — `/announce`: "+ Nueva persona" resuelve el Teléfono/WhatsApp EN VIVO (mismo espíritu que el campo único principal) -- si ya es Persona conocida, oculta el campo Nombre y usa su nombre real; distingue "ya vive aquí" (bloquea), "ya vive en otra unidad" (pre-marca mudanza) y "conocida sin unidad" -- de paso corrige un bug real: el servidor exigía Nombre incluso cuando iba a ignorarlo — **implementado, pendiente desplegar y confirmar**
- `337` — Menú de cuenta (`.account-menu-panel`, categorías del issue 336): "Datos y catálogos"→"Datos", "Equipo y comunicación"→"Perfiles", ítem "Perfiles"→"Usuarios"; "Proveedores" y "Motivos de anulación" se mudan al panel "Datos" — **implementado, pendiente que el cliente lo confirme visualmente**
- `338` — `/administracion/contactos-externos`: muestra la cantidad total de contactos junto a la paginación, incluso con 1 sola página (donde el componente compartido no renderiza nada) — **implementado, verificado en vivo en localhost:8010**
- `339` — `/paquetes`: íconos de la columna Acciones con el mismo `chip_icono` (círculo relleno de color) que ya usa `/residentes`, en vez del estilo propio "solo ícono, sin caja"; ronda 2: tamaño también igualado (mismo `clamp` responsive, antes quedaba fijo en 36px); ronda 3: Acción/Cancelar en estados terminales ya no se quedan en rojo, mismo apagado gris que el resto — **implementado, pendiente confirmar visualmente**
- `340` — `/paquetes` y `/residentes`: renombres de columnas -- "Dirección"→"Torre y Apartamento" y "Cliente"→"Residente" en `/paquetes`; "Teléfono de contacto"→"Contacto" y "Nombre"→"Residente" en `/residentes` — **implementado, pendiente confirmar visualmente**
- `341` — `/paquetes` mobile: solo columnas Residente y Acciones visibles; Torre y Apartamento + Fecha bajan a píldoras bajo el nombre (mismo patrón que `/residentes`, issues 277/278/281) — **implementado, pendiente confirmar visualmente**
- `342` — `/paquetes` modal "Ver": íconos "Modificar" y "Asignar apartamento" agregados junto al botón Recibir/Entregar, mismo tamaño que en sus ubicaciones originales — **implementado, pendiente confirmar visualmente**
- `343` — `/paquetes`: columna "Fecha" retirada y "Asignar apartamento" quitado de la fila (ambos consultables en el modal "Ver"); en mobile, columna Acciones limitada a WhatsApp/Teléfono/Recibir-Entregar — **implementado, cliente reportó posible bug relacionado, en investigación**
- `344` — `/paquetes`: íconos de filtro de Estado (Anunciado/Recibido/Entregado/Cancelado) + "Quitar filtros" redistribuidos en todo el espacio disponible (antes: grupo de 4 pegados vs. X al extremo), mismo tamaño — **implementado, pendiente confirmar visualmente**
- `345` — `/paquetes` mobile: ícono de Apartamento (🏠 apagado) quitado por completo de la píldora cuando no hay unidad asignada — **implementado, pendiente confirmar visualmente**
- `346` — "Asignar apartamento" restaurado como activo en desktop para Anunciado/Recibido sin unidad (efecto colateral de issue 343); mobile confirmado sin ícono en ningún estado — **implementado, pendiente confirmar visualmente**
- `347` — Modales: en mobile, siempre empiezan a la misma altura (antes: centrado vertical dependía de la altura de cada modal) — **implementado, pendiente confirmar visualmente**
- `348` — `/paquetes`: fondo de fila según el estado (ámbar/azul/esmeralda/rojo), para diferenciar paquetes de un vistazo; 4 rondas de ajuste (borde para filas consecutivas del mismo estado, intensidad subida y luego bajada a un punto intermedio con opacidad) — **implementado, pendiente confirmar visualmente**
- `349` — `/paquetes`: aro blanco en los íconos de Acciones para que contrasten contra el nuevo fondo de fila — **implementado, pendiente confirmar visualmente**
- `350` — Info legal/cambiante (NIT, dirección, razón social, etc.) hoy hardcodeada en 4 plantillas distintas debería ser editable desde `/administracion/conjunto` y reflejarse en las páginas públicas (misma idea para tarifas, que ya tienen fuente de verdad real en `/administracion/tarifas-cobro`) — **desplegado a test.papyrus.com.co (2026-09-19), pendiente que el cliente lo confirme visualmente**
- `351` — `/anunciar`: el aviso de privacidad bajo el checkbox de Términos y Condiciones tenía tipografía distinta (más chica, más clara) al texto del checkbox mismo — unificados al mismo tamaño/color; ronda 2: pedido de dejarlos como UN solo elemento en vez de `<label>` + `<p>` separados — fusionados en un solo `<label>` (el aviso de privacidad va en un `<span class="block">` interno) — **implementado**
- `352` — `/administracion/conjunto`: los campos no mostraban ninguna etiqueta visible (el texto del label solo se usaba como placeholder, que desaparece en cuanto el campo ya tiene un valor -- y TODOS los campos de esta pantalla arrancan con valor real) -- nuevo parámetro `mostrar_label` en `input_texto()` (componente compartido, ~25 usos), activado en las 10 filas de esta pantalla — **implementado**
- `353` — `/administracion/contactos-externos`, "Importar CSV": bug real -- si todavía no hay ningún contacto importado, la lista de fuentes existentes queda vacía y "Otra (especificar)…" termina siendo la ÚNICA opción del selector, pero el campo "Nombre de la fuente nueva" solo se revela con un evento `change` del selector, nunca al cargar la página -- bloqueaba el primer import de cualquier ambiente nuevo — **implementado**

...
- `322` — OTP (fuga de código en logs del monolito legado, no desplegado) y código de acceso (nunca visibles en vistas públicas no autenticadas -- `/anunciar/confirmacion` los exponía en pantalla y en la URL) — **implementado, pendiente desplegar y confirmar**
- `323` — `/paquetes`: buscar un código de acceso EXACTO trae también los paquetes `RECIBIDO` relacionados (mismo destinatario o misma unidad), para entregar todo de una sola consulta — **implementado, pendiente desplegar y confirmar**

...
- `277` — `/residentes` tabla (mobile): columna Teléfono oculta, Torre y Apartamento en formato compacto ("T05-102"), nombre truncado, íconos de Acciones más chicos -- 0px de scroll lateral en un viewport de 359px (era 101px con solo los 2 cambios pedidos; el resto se agregó al verificar en vivo que no alcanzaba), desktop sin cambios — **implementado**
- `278` — Seguimiento a [[277]]: badges Auto/Principal bajan a su propia fila en mobile (`sm:contents`), nombre con ancho `clamp()` fluido (no un solo breakpoint, pedido explícito "lo mas ajustable posible"), íconos de Acciones con `clamp()` 26px→32px -- 0px de scroll desde 360px en adelante, desktop sin cambios — **implementado**
- `279` — Seguimiento a [[278]]: se retira el truncado "…" del nombre -- vuelve a mostrarse completo; en la práctica solo elimina el scroll lateral desde ~414px de ancho (a 360-390px reaparece, proporcional al largo del nombre), reportado al cliente con los números — **implementado**
- `280` — Seguimiento a [[279]]: nombre en mobile acotado a 2-3 palabras completas (nunca caracteres/"…") -- 3 palabras si esas 3 juntas no pasan de 20 caracteres, si no 2; nuevo helper `_nombre_mobile`, no toca el nombre real en base de datos, desktop sin cambios — **implementado**

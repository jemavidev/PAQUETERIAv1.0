#!/bin/bash
# ============================================================================
# Script: Crear Base de Datos Staging
# Descripción: Crea la base de datos paqueteria_staging en AWS RDS
# Uso: ./create_staging_database.sh
# ============================================================================
...
DB_EXISTS=$(PGPASSWORD=$PROD_PASS psql \
  -h $PROD_HOST \
  -U $PROD_USER \
  -d postgres \
  -t \
  -c "SELECT 1 FROM pg_database WHERE datname='$NEW_DB';" 2>/dev/null | tr -d ' ')
...
PGPASSWORD=$PROD_PASS psql \
  ...
  -U $PROD_USER \
  -d postgres \
  -c "CREATE DATABASE $NEW_DB OWNER $PROD_USER;"
...
DB_SIZE=$(PGPASSWORD=$PROD_PASS psql \
  -h $PROD_HOST \
  -U $PROD_USER \
  -d postgres \
  -t \
  -c "SELECT pg_size_pretty(pg_database_size('$NEW_DB'));" 2>/dev/null | tr -d ' ')
...
echo "      docker-compose -f docker-compose.staging.yml config"
echo ""

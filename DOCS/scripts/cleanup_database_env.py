...
# -*- coding: utf-8 -*-
"""
PAQUETES EL CLUB v1.0 - Script de Limpieza de Base de Datos (Variables de Entorno)
Versión: 1.0.0
Fecha: 2025-01-24
...
class DatabaseCleanup:
    ...
    def __init__(self):
        """Inicializar conexión a la base de datos usando variables de entorno"""
        self.connection = None
        self.cursor = None
        
        # Cargar variables de entorno desde CODE/LOCAL/.env si existe
        env_file = Path("CODE/LOCAL/.env")
        if env_file.exists():
            ...
            logger.info("Variables de entorno cargadas desde CODE/LOCAL/.env")
        
        # Configurar conexión
        self.db_config = {
            'host': os.getenv('DB_HOST', 'localhost'),
            'port': os.getenv('DB_PORT', '5432'),
            'database': os.getenv('DB_NAME', 'paqueteria'),
...
def main():
    """Función principal"""
    print("🚀 PAQUETES EL CLUB v1.0 - Script de Limpieza de Base de Datos")
    print("="*60)
    
    # Verificar que estamos en el directorio correcto
    if not (Path.cwd() / "CODE" / "LOCAL").exists():
        print("❌ Error: Ejecutar este script desde la raíz del proyecto")
        print("💡 Ejecuta: python SCRIPTS/database/cleanup_database_env.py")
        sys.exit(1)

...
if [ ! -d "CODE/LOCAL" ]; then
    error "Ejecutar este script desde la raíz del proyecto"
    echo "💡 Ejecuta: ./SCRIPTS/database/cleanup_database.sh"
    exit 1
fi
...
if [ -f "CODE/LOCAL/.env" ]; then
    ...
    log "Variables de entorno cargadas desde CODE/LOCAL/.env"
else
    error "Archivo CODE/LOCAL/.env no encontrado"
    exit 1
fi
...
DB_NAME=${POSTGRES_DB:-paqueteria_local}
DB_USER=${POSTGRES_USER:-paqueteria_user}
DB_PASSWORD=${POSTGRES_PASSWORD:-paqueteria_pass}
...
confirm_cleanup() {
    echo
    echo "============================================================"
    echo "⚠️  ADVERTENCIA: LIMPIEZA DE BASE DE DATOS  ⚠️"
    echo "============================================================"
...
# Función principal
main() {
    echo "🚀 PAQUETES EL CLUB v4.0 - Script de Limpieza de Base de Datos"
    echo "============================================================"
    
    # Verificar conexión a la base de datos
    if ! psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -c "SELECT 1;" >/dev/null 2>&1; then
        error "No se puede conectar a la base de datos"
        echo "💡 Verifica las variables de entorno en CODE/LOCAL/.env"
        exit 1
    fi
